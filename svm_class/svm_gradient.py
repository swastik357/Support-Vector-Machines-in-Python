...
class SVM:
  ...
  def _train_objective(self):
    return np.sum(self.alphas) - 0.5 * np.sum(self.YYK * np.outer(self.alphas, self.alphas))  # maximize this
  ...
  def fit(self, X, Y, lr=1e-5, n_iters=400):
    ...
    plt.plot(losses)
    plt.title(f'train objective per iteration for {selectedData}')
    plt.show()
...
if __name__ == '__main__':
  Datasets = [medical,medical_sigmoid,xor,donut,spiral,clouds]
  for selectedData in Datasets:
      print('************************************************')
      print(f'PROCESSING DATA {selectedData}')
      print('************************************************')
      Xtrain, Xtest, Ytrain, Ytest, kernel, lr, n_iters = selectedData()
      ...
      if Xtrain.shape[1] == 2:
        plot_decision_boundary(model,selectedData)
