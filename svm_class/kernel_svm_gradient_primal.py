...
class KernelSVM:
  ...
  def fit(self, X, Y, lr=1e-5, n_iters=400):
    ...
    plt.hist(m, bins=20)
    plt.title(f'histogram of margins for {selectedData}')
    plt.show()

    plt.plot(losses)
    plt.title(f'loss per iteration for {selectedData}')
    plt.show()
...
if __name__ == '__main__':
  DataSets = [clouds,medical,xor,donut,spiral]
